...
SPELL_COSTS := []int { 0, 53, 73, 113, 173, 229 }
...
playable_actions :: proc(state: Game_State, actions: ^[dynamic]Action) {
  ...
  if state.player_mana >= SPELL_COSTS[Action.SHIELD] && state.shield_turns == 0 {
    append(actions, Action.SHIELD)
  }

  if state.player_mana >= SPELL_COSTS[Action.POISON] && state.poison_turns == 0 {
    append(actions, Action.POISON)
  }

  if state.player_mana >= SPELL_COSTS[Action.RECHARGE] && state.recharge_turns == 0 {
